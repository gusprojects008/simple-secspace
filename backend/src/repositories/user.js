...
async function create(username, email, passwordHash) {
  ...
  const result = await query(query, values); // You could use {rows} to get the result
...
async function findById(user_id) {
  const result = await db.query('SELECT id, username, email, password_hash FROM users WHERE id = $1', [user_id]);
  return result.rows[0];
};
